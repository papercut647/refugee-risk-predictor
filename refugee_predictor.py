...
@st.cache_data
def forecast_future(df, _clf, _reg, _scaler, features):
    """Project indicators forward to 2030 and predict."""
    ...
    for _, row in base.iterrows():
        for i, yr in enumerate(future_years, 1):
            ...
            proj["fragile_state_idx"]   = min(120, proj["fragile_state_idx"] + i * 0.5)
            proj["gdp_per_capita"]     *= (1 - proj["conflict_intensity"] * 0.02) ** i
            rows.append(proj)

    future_df = pd.DataFrame(rows)
    X_f = _scaler.transform(future_df[features])
    future_df["risk_class"]        = _clf.predict(X_f)
    future_df["displaced_persons"] = _reg.predict(X_f).astype(int).clip(min=0)
    future_df["risk_score"]        = _reg.predict(X_f) / _reg.predict(X_f).max() * 0.95
...
future_df = forecast_future(df, clf, reg, scaler, features)
...
with st.sidebar:
    ...
    st.caption("**Climate:** IPCC projections, EM-DAT flood data, CHIRPS drought index")
    st.caption("**Conflict:** ACLED, UCDP Armed Conflict Dataset")
    st.caption("**Economic:** World Bank WDI, FAO Food Security")
    st.caption("**Governance:** Fund for Peace FSI, Transparency International")
    st.markdown("---")
    st.caption("⚠️ Built on synthetic representative data. For research/planning use.")
...
with tab2:
    ...
    if sel_countries:
        ...
        fig_trend = px.line(
            ...
            title="Displacement Trend + Forecast",
    ...
    # Climate vs Conflict drivers
    st.subheader("🔀 Climate vs. Conflict Drivers Over Time")
    driver_df = full_df.groupby("year").agg(
        climate_displaced=("displaced_persons", lambda x:
            full_df.loc[x.index][full_df.loc[x.index, "flood_risk"] > 0.5]["displaced_persons"].sum()),
        conflict_displaced=("displaced_persons", lambda x:
            full_df.loc[x.index][full_df.loc[x.index, "conflict_intensity"] > 0.5]["displaced_persons"].sum())
    ).reset_index()

    fig_drivers = go.Figure()
    fig_drivers.add_trace(go.Scatter(x=driver_df["year"], y=driver_df["climate_displaced"],
        name="Climate-Driven", fill="tozeroy", line=dict(color="#4da6ff"), stackgroup="one"))
    fig_drivers.add_trace(go.Scatter(x=driver_df["year"], y=driver_df["conflict_displaced"],
        name="Conflict-Driven", fill="tonexty", line=dict(color="#ff6b6b"), stackgroup="one"))
    fig_drivers.add_vrect(x0=2025.5, x1=2030, fillcolor="#ffffff", opacity=0.04,
                          annotation_text="Forecast", annotation_position="top left")
    fig_drivers.update_layout(
        title="Global Displacement: Climate vs Conflict Overlap",
        template="plotly_dark", height=320,
        xaxis_title="Year", yaxis_title="Displaced Persons"
    )
...
with tab4:
    ...
    | Displacement Volume | Gradient Boosting Regressor | Predicts displaced persons count |
    | Forecast Horizon | 2026–2030 | Climate indicators trend-projected; conflict held near 2025 baseline |
